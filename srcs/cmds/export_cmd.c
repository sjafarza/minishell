...
/*   Updated: 2022/01/27 15:22:31 by scarboni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	prepare_var_elements(t_env *env, const char **args,
	char *strings[MAX_STRINGS], int i)
{
	int	k;

	if (produce_name_value((char *)args[i], &strings[STRING_NAME],
			&strings[STRING_VALUE]) == -EXIT_FAILURE)
		return (strings_clean(strings, -EXIT_FAILURE, args[i],
				"not valid identifier"));
	if (ft_strlen(strings[STRING_NAME]) <= 0)
		return (strings_clean(strings, -EXIT_FAILURE, args[i],
				"var name must contain at least 1 character"));
	k = -1;
...
int	export_cmd(t_env *env, const char *cmd, const char **args)
{
	...
	while (args[++i])
	{
		if (prepare_var_elements(env, args, strings, i) != EXIT_SUCCESS)
			return (1);
				...
				strings[STRING_VALUE]) == EXIT_SUCCESS)
		{
			...
			return (strings_clean(strings, 1, NULL, NULL));
	}
	return (EXIT_SUCCESS);
}
