...
/*   Updated: 2022/02/03 08:58:51 by scarboni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const
	t_cmd g_cmd_dictionary[MAX_CMD] = {
...
int			parse_simple_quote(t_line *line_handle, t_tmp_parsed *tmp_parsed,
				t_parse_utils p_utils);
int			parse_type_without_arg(t_line *line_handle,
...
static const
	t_parser g_parser_dictionary_for_doubles_quotes[MAX_PARSER] = {
...
t_env_var	*init_empty_env_var(t_env *env, char const *name,
				char const *value);
...
int			find_and_update_env_var(t_env *env, char *var_name,
				char *new_value);
...
int			move_env_vars_array_content(t_env_var *src, t_env_var *dst,
				int len);
...
int			check_parsing(const t_parser dictionnary[MAX_PARSER],
				t_line line_handle, t_tmp_parsed *tmp_parsed,
				t_parse_utils p_utils);
int			is_sequence_equal_to_parser_code(int type_code, char *seq);
int			add_new_env_by_value_name_raw(t_env *env, char *name, char *value,
				char *raw);
int			produce_name_value(char *var, char **name, char **value);
char		**get_raw_env_array(t_env *env);
char		**init_array_with_one_str(char *s);
int			init_array_once_ready(t_line line_handle, t_tmp_parsed tmp_parsed,
				int i, int did_find_parsing);
...
t_norm_loop	extract_next_arg_loop(t_env *env, t_line line_handle,
				t_tmp_parsed *tmp_parsed, t_norm norm);
int			extract_next_arg(t_env *env, t_line line_handle,
				t_tmp_parsed tmp_parsed, int do_not_parse_until);
